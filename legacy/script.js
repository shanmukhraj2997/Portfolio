    ...
    const setupSkills = () => {
        const createSkillPill = (skill) => `
            <span class="glass-card hover:border-blue-500 transition-all duration-300 text-slate-700 dark:text-slate-200 py-2 px-5 rounded-xl text-sm font-bold shadow-sm flex items-center gap-2">
                <i class="${skill.icon} text-blue-500"></i> ${skill.name}
            </span>
        `;

        document.getElementById('list-languages').innerHTML = categorizedSkills['Languages'].map(s => createSkillPill(s)).join('');
        document.getElementById('list-web-tech').innerHTML = categorizedSkills['Web Technologies'].map(s => createSkillPill(s)).join('');
        document.getElementById('list-frameworks').innerHTML = categorizedSkills['Frameworks'].map(s => createSkillPill(s)).join('');
